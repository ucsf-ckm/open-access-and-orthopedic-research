...
function processData (data) {
  ...
  for (const record of records) {
    ...
    let correspondingAuthorIndex = [];
    if (Array.isArray(name)) {
      line.push(name.filter((nameData) => nameData.role === 'author').map((nameData) => nameData.display_name).join('; '))
      correspondingAuthorIndex = name.filter((nameData) => nameData.role === 'author' && nameData.reprint === 'Y').map((nameData) => nameData.addr_no)
    } else if (name.display_name) {
      line.push(name.display_name)
      if (name.reprint === 'Y') {
        if (!name.hasOwnProperty('addr_no')) {
          throw new Error(`addr_no is not defined: ${JSON.stringify(name)}`)
        }
        correspondingAuthorIndex.push(name.addr_no)
      }
    } else {
    ...
    let affiliations = ''
    let countries = ''
    let correspondingAuthorCountry = ''
    if (addressName) {
      if (Array.isArray(addressName)) {
        ...
        correspondingAuthorCountry = addressName.filter((addressData) => correspondingAuthorIndex.includes(addressData.address_spec.addr_no))
                                                .map((addressData) => addressData.address_spec.country).join('; ')
      } else {
        ...
        if (correspondingAuthorIndex.length) {
          correspondingAuthorCountry = countries;
        }
      }
    }
    ...
    // Corresponding author country
    line.push(correspondingAuthorCountry)
